 ...
 */
export async function GET(_: Request, { params }: { params: { id: string; message: string } }) {
  ...
  // a specific asset with id = params.id and with the message params.message

  return NextResponse.json({ accessRequestSent: true, asset: params.id, message: params.message })
}
